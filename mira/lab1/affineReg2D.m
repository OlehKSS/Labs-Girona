...
mtype = 'cc'; % metric type: sd: sum of square differences;
% ssd gcc: gradient correlation; cc: cross-correlation
...
aff_param = @(x)affine_registration_function(x,scale,Im,If,mtype,ttype);
opt_param = optimset('Display','iter','MaxIter',1000, 'TolFun', 1.000000e-10,'TolX',1.000000e-10, 'MaxFunEvals', 1000*length(x), 'PlotFcns', @optimplotfval);
    
[x]=fminsearch(aff_param, x, opt_param);
